...
from webapp.verbatim import (  # noqa: E402
    hidden_spans, install as install_verbatim, marking, text_lines,
    verbatim_blocks,
)
...
# Characters per chunk sent for translation. Bigger means more context per
# paragraph; smaller means fewer segments per reply, which a model without
...
def _run_job(job_id: str, src: Path, api_key: str, model: str, lang_in: str,
    ...
    try:
        ...
        with GATE.slot():
            ...
            # Text the source had painted over must not be
            # revived by re-drawing it above the graphics.
