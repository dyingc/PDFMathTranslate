...
def _preamble(tr, profile: dict) -> str:
    ...
    Do not pad this out to reach a provider's minimum cacheable length. It was
    tried, on the reasoning that OpenAI caches nothing below 1024 tokens and
    this comes to 445, so on that provider it had never once been cached. The
    gap was filled with the document's own opening, fenced off as reference.
    Measured on a 214-paragraph paper: input per call went 627 -> 1764 exactly
    as intended, cache hits stayed at 1792 of 144682 tokens, and the bill went
    up 12% for nothing. Translation quality was unharmed — no chunk came back
    misaligned — so the excerpt is a viable way to carry context if that is ever
    wanted. It is simply not a way to save money.

    The reason, isolated by changing one variable at a time: the endpoint in use
    caches whole requests, not shared prefixes. Same prefix and same body hits;
    same prefix and a different body misses, in plain text and in JSON mode
    alike. Our calls all share a prefix and differ after it, so they can never
    hit — the 1792 that did was a failed chunk being retried verbatim. Padding
    a prefix only pays where the prefix itself is the cache key, and that is
    worth re-measuring rather than assuming before anyone tries this again.
    """
