...
匹配按**词干**、且忽略大小写（缩写除外——对 `TIP` 来说大小写是它和 `tip` 唯一的区别）。
这两条都是在 SPA.pdf 上付了代价才学到的：
...
所以规则里明确写了：一条术语覆盖它的**所有语法形式**，`sound` / `unsound` /
`soundly` 必须建立在同一个词根上。这一层交给模型判断，因为形态学是它懂的东西，
而词干匹配只负责保证这条术语**能被送到它面前**。
