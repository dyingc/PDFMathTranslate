...
重跑时连"这些段落是不是已经缓存过"都得先花钱问一遍文档是什么。所以描述结果按
**文件内容的 sha256** 存在 `data/profiles/` 下。

用内容哈希而不是文件名：文件名会撞（不同内容都叫 `SPA.pdf`），也会变（改个名就成了
新文档）。内容哈希两头都对，改过一个字的文件会被重新描述。

实测同一文件跑两次：第一次 1 次描述调用 + 4 次块调用，第二次 **0 次调用**。
